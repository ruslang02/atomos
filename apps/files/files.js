	...
	if (!Object.keys(registry.get()).length) {
		let defaultFile = await fsp.readFile(__dirname + "/default.json");
	...
	async function navigate(url) {
		...
		async function generateItem(file) {
			...
			item.openProperties = function() {
				...
				icon.src = item.isDirectory ? path.join(osRoot, "icons", "Extensions Folder.png") : path.join(osRoot, "icons", "file-types", "Unknown.png");
				icon.className = "mr-1";
			...
			item.compress = function() {
				...
				shell.selectFile(shell.ACTION_SAVE, {
					...
					require('tar').create({
						...
						file: file,
						C: item.path
					}, await fsp.readdir(item.path)).then(e => new Notification({
						app: "Archive Manager",
						icon: "archive",

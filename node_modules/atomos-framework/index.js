...
window.Tray = class {
  constructor(glyph, title = "") {
    this.glyph = glyph;
    this.title = title;
    this.onNotificationClick = jQuery.noop;
    this.onClick = jQuery.noop;
    this.onContextClick = jQuery.noop;
  ...
  show() {
    ...
    ipcRenderer.on('tray-click', this.onClick)
    ipcRenderer.on('tray-context-click', this.onContextClick)
    ipcRenderer.on('notification-click', this.onNotificationClick)
  ...
  notify(title, html = "", state = "info", timeout = 10000, after = "REMOVE") {
    remote.BrowserWindow.fromId(6).webContents.send("spawn-notification", {
      html: html,
      title: title,
      state: state,
      timeout: timeout,

...
module.exports.registry = {
	on: function(event, callback, isSystem = false) {
		let previousSR;
		setInterval(function() {
			module.exports.registry.get(isSystem).then((regs) => {
				if (!deepEqual(regs, previousSR))
					callback(regs);
				previousSR = regs;
			})
		}, 1000);
		...
		return new Promise((resolve = console.log, reject = console.error) => {
			this.get(isSystem).then(function(rItem) {
				...
				systemRegistry.set({
					...
					name: (isSystem ? "system" : module.exports.app.name),
	...
	get: function(system = false) {
		return new Promise((resolve = console.log, reject = console.error) => {
			systemRegistry.get({
				domain: "atomos.registry",
				url: "http://atomos.registry/",
				name: (system ? "system" : module.exports.app.name)
			}, function(e, rItem = []) {
				if (e) reject(e);
...
module.exports.registry.on("changed", (regs) => {
	wc.setZoomFactor(regs.uiScaling);
}, true);
...
module.exports.Notification = class {
	...
	constructor(options = {}) {
		...
		options.icon = options.icon || wc.browserWindowOptions.icon; // Some apps still use a custom notification icon. I will not admit this.
		module.exports.registry.get(true).then((reg) => {
			let nSettings = reg.notificationPrefs[module.exports.app.name];
			if (!nSettings) {
				reg.notificationPrefs[module.exports.app.name] = [1, 1]
				nSettings = [1, 1];
				module.exports.registry.set({
					notificationPrefs: Object.assign({}, reg.notificationPrefs, {
						[module.exports.app.name]: [true, true]
					})
...
module.exports.app = {
	...
	launch: function(prog, args, electronOptions) {
		return new Promise(function(resolve, reject) {
			 ...
			 */
			let dApps = fs.readJsonSync(path.join(app.getPath("appData"),
				"defaultApps.json"), {
				throws: false
			});
			if (dApps === null)
				fs.writeFile(path.join(app.getPath("appData"), "defaultApps.json"), fs.readFileSync(
					path.join(app.getAppPath(), "resources", "defaultApps.json.template")
				));
			else if (dApps[prog])
				prog = dApps[prog].app;
	...
	isDefault: function(genericName) {
		return fs.readJsonSync(path.join(app.getPath("appData"), "defaultApps.json"))[
			genericName].app === module.exports.app.name;
	},
	setDefault: function(genericName) {
		...
		if (dApps === null)
			fs.writeFile(path.join(app.getPath("appData"), "defaultApps.json"), fs.readFileSync(
				path.join(app.getAppPath(), "resources", "defaultApps.json.template")));
		else if (dApps[genericName]) {
			dApps[genericName].app = module.exports.app.name;
			fs.writeJson(path.join(app.getPath("appData"), "defaultApps.json"), dApps);
		}
	},
	fileOpen: function(url) {
		...
		fs.readJson(app.getPath("appData") + "/associations.json", "utf8", function(
			err, assocs) {
			if (err) {
				assocs = [];
				fs.writeJsonSync(app.getPath("appData") + "/associations.json", []);
			}
			assocs = assocs.find(o => o.extension == extension);
			if (assocs) module.exports.app.launch(assocs.app, {
				path: url
			});
			else module.exports.app.launch("aos-appchooserdialog", {
				path: url,
				extension: extension
			});
		});
